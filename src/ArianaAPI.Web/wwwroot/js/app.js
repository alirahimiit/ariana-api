...
const App = {
    ...
    // ⭐ تبدیل پاسخ بک‌اند به فرمت revenues/expenses
    _adaptSoodZiyanResponse(data) {
        const groups = data.groups || [];

        // گروه‌های درآمد: 3 (فروش) و 6 (درآمد)
        // گروه‌های هزینه: 4 (خرید) و 5 (هزینه)
        const revenues = groups
            .filter(g => g.groupTypeCode === 3 || g.groupTypeCode === 6)
            .map(g => this._adaptSoodZiyanGroup(g, 'revenue'));

        const expenses = groups
            .filter(g => g.groupTypeCode === 4 || g.groupTypeCode === 5)
            .map(g => this._adaptSoodZiyanGroup(g, 'expense'));

        const totalRevenue = revenues.reduce((s, g) => s + g.total, 0);
        const totalExpense = expenses.reduce((s, g) => s + g.total, 0);
        ...
        return {
            ...
            totalExpense,
            netProfit: net,
            resultType: net > 0 ? 'profit' : (net < 0 ? 'loss' : 'equal'),
            resultText: net > 0 ? 'سود' : (net < 0 ? 'زیان' : 'سر به سر')
        };
    },

    _adaptSoodZiyanGroup(g, nature) {
        // ─── هر آیتم ───
        const items = (g.items || []).map(it => ({
            codeCol: it.codeCol,
            hesabName: it.hesabName,
            mabBed: it.mabBed || 0,
            mabBes: it.mabBes || 0,
            // مانده = Bed - Bes (مثبت = بد، منفی = بس)
            mabMan: (it.mabBed || 0) - (it.mabBes || 0)
        }));

        const totalMabBed = items.reduce((s, x) => s + x.mabBed, 0);
        const totalMabBes = items.reduce((s, x) => s + x.mabBes, 0);
        const total = totalMabBed - totalMabBes;

        return {
            nature,                          // 'revenue' یا 'expense'
            groupTypeName: g.groupName,      // ← مپ کردن groupName به groupTypeName
            groupTypeCode: g.groupTypeCode,
            items,
            totalMabBed,
            totalMabBes,
            total
